    ...
    >
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', gap: Spacing.sm }}>
        ...
        <View style={{ flex: 1, minWidth: 0 }}>
          ...
          <Text style={[Type.display, { color: c.fg }]} numberOfLines={1} adjustsFontSizeToFit>{title}</Text>
          {!!subtitle && <Text style={[Type.body, { color: c.fg2, marginTop: Spacing.xs }]} numberOfLines={2}>{subtitle}</Text>}
        </View>
        {right && <View style={{ alignSelf: 'center', flexShrink: 0 }}>{right}</View>}
      </View>
    </LinearGradient>
  );
}
